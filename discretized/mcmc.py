...
def errorFunction(theta, y0, tVector, data, sigma, allParams):
    ...
    estC = np.array(estC)
    estI = np.array(estI)
    # estC = y.iloc[:,3*numberUnits:4*numberUnits].values
    # estI = y.iloc[:,4*numberUnits:5*numberUnits].values
...
def main():
    ...
    avgC = np.array([data[:int(len(tVector)//24)] for data in allParams['fitData']['C_fitData']])
    avgI = np.array([data[:int(len(tVector)//24)] for data in allParams['fitData']['I_fitData']])
    obs = [avgC, avgI]
    # create our Op
...
def plotCalibratedModel():
    ...
    dataC = [item[:len(tVector)//24] for item in allParams['fitData']['C_fitData']]
    dataI = [item[:len(tVector)//24] for item in allParams['fitData']['I_fitData']]
    C,I = [],[]
